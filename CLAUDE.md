...
`parseRange`, `windowFromRange`, `parseDur`, `midColor` exist twice: once in `src/chartLogic.ts` (TS, used by extension and `webview.ts`) and once as inline JS inside the `src/chartView.ts` template (used by the chart panel webview). When changing any of them, update BOTH copies in the same change.
