...
After install, run `Developer: Reload Window` (the command palette is `Ctrl+Shift+P` / `Cmd+Shift+P`). VS Code does not auto-reload extensions on update.
...
Chart options (day range, gap, gradient colors, forecast, focus, Y-axis mode, etc.) live inside each chart panel and persist across reloads. Sidebar-only toggles (USD annotations, VS Code theme skin) live in the settings dropdown at the bottom of the sidebar.
...
- Chart and sidebar UI state lives in VS Code's per-extension `webviewState` and `globalState` — also entirely local.
...
VS Code does not reload installed extensions automatically. Run `Developer: Reload Window`. If a chart panel was open, close and reopen it.

**Chart says "Invalid range" for an obvious input like `1`.**
Open and re-open the chart panel after a reload. If the issue persists, uninstall and reinstall the extension to reset persisted webview state.
...
Source, contribution guide and detailed development notes live on [GitHub](https://github.com/Avhatar/ClaudeCodeVsCodeStatisticsPlugin).
