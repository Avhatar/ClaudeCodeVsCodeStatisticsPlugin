...
## [0.67.0] - 2026-05-10
### Added
- **Node.js detection + diagnostics flow.** User report: brand-new install, "Install hook" button clicked, sidebar said "Hook installed, waiting for first turn", several turns went by, no data ever appeared. Root cause analysis identified ≥10 possible silent failure modes; the most pressing (and the one we now know was hitting the user) is that the plugin assumed `node` was on PATH without ever checking. With Claude Code shipping a native standalone installer that doesn't pull Node as a dependency, the "CC user has Node" invariant we'd relied on since v0.0 no longer holds. Symptoms identical to the user's: hook registered in `settings.json`, but the `node "..."` command silently fails to spawn under Claude Code's hook shell, never writes the invocation log, never writes `usage-log.txt`, sidebar stays in waiting state forever.
- New helper `findNodeExecutable()` in [hookSetup.ts](src/hookSetup.ts) — runs `where node` (Win) / `which node` (POSIX) via `spawnSync` (timeout 2s), then verifies with `<path> --version`. Returns `{ path, version }` or null. Used to (a) gate the install flow, (b) record the absolute path in the registered command so PATH-divergence between VS Code and CC's hook shell stops being a silent failure, (c) drive the sidebar "Node detected" hint and "Install Node.js" CTA.
- **Inline diagnostic banner in the sidebar.** `installHook()` now returns a structured `InstallResult` (`{ ok, changed, status, error?, nodePath? }`) instead of throwing, with a discriminated `InstallError` union covering: `no-node`, `bundled-missing`, `script-copy`, `settings-read`, `settings-write`. The extension parks the most recent error in `lastInstallError` and the sidebar's `renderInstallErrorPanel` renders a tailored panel per code, with bespoke remediation hints (permissions, AV interference, malformed JSONC, etc.) instead of the previous transient toasts. Toasts disappear while the user is in another window; an inline panel doesn't.
- Three new commands wired to the diagnostic UI:
  - `claudeUsage.openNodeJsInstall` — opens nodejs.org in the user's browser via `vscode.env.openExternal`.
  - `claudeUsage.checkNode` — re-runs `findNodeExecutable()` (forced), clears any sticky `no-node` error, refreshes the sidebar; toast tells the user Node was found and they can install the hook now.
  - `claudeUsage.openSettingsJson` — opens `~/.claude/settings.json` in an editor tab. Used by the "settings-read" / "settings-write" diagnostic panels.

### Changed
- **Hook command rewritten with absolute Node path.** Previously `node "C:/Users/.../hook.js"`; now `"C:/Program Files/nodejs/node.exe" "C:/Users/.../hook.js"`. Existing 0.66.x installations are upgraded in place: `installHook()` finds the existing entry by marker, compares the recorded `command` string to what we'd write today, rewrites it if different. No "remove and reinstall" required from the user.
- **`maybePromptSetup` is now node-aware.** When node is missing, the setup prompt swaps from `Install hook / Open settings.json / Don't ask again` to `Install Node.js / I have Node, retry detection / Don't ask again`. Sending the user to "Install hook" before they have node was a dead-end loop (install would refuse, sidebar would surface no-node panel, they'd be back here).
- Cached `cachedNodeInfo` lives in extension memory across `refresh()` calls — first refresh probes once via spawnSync, subsequent refreshes pass the cached result through unchanged. Re-probing on every render would jitter the sidebar on slow systems and burn child-process cost for no reason. The `Recheck` action and successful install paths force a re-probe.

### Fixed
- **`readSettings()` no longer silently coerces unparseable JSON to `{}`.** The pre-0.67 implementation called `JSON.parse(raw)` and on any throw returned `{}`. If the user's `settings.json` had JSONC features (comments, trailing commas — Claude Code accepts these, our strict `JSON.parse` doesn't), or was corrupted/truncated, we'd silently treat the file as empty, then `installHook()` would `writeSettings({hooks:{Stop:[ours]}})` on top, **wiping any pre-existing settings** (other tools' hooks, permissions blocks, env, etc.). New behaviour: `readSettings()` returns a discriminated `{kind:'ok', settings} | {kind:'parse-error', detail}` union; on parse-error the install refuses and the sidebar shows a `settings-read` diagnostic with an "Open settings.json" button. Same protection added to `uninstallHook` (refuses to touch malformed file).
- All `try/catch` blocks around `fs.writeFileSync` / `fs.readFileSync` / `fs.mkdirSync` in `installHook()` now propagate the OS error message into a structured `InstallError` instead of throwing into the extension's outer toast handler. The `permissionsHint(detail)` helper in webview.ts inspects the detail string for `EACCES` / `EPERM` / `EBUSY` / `ENOSPC` / `EROFS` and produces a tailored remediation paragraph per error class.
- Replaced `installHook`'s old `for (... some ...)` registration check with `findIndex` + in-place command rewrite, fixing a pre-existing latent issue where if the user's `settings.json` already had a `Stop` entry containing our marker but with a stale command (e.g. a different absolute path from a previous extension install location), we wouldn't update it and the user would silently keep running the old binary.

### Implementation notes
- `ViewState` extended with two fields: `nodeInfo: NodeInfo | null | false` (null = not yet checked, NodeInfo = found, false = checked-and-missing) and `lastInstallError: InstallError | null`. Both are passed through `refresh()` into the renderer.
- `renderInstallErrorPanel(err, afterInstall=false)` is reachable from two paths: (1) we just attempted install and bounced (`lastInstallError != null`), (2) we haven't attempted yet but already know node is missing (`nodeInfo === false` + `err.code === 'no-log'` + `!hookRegistered`). The `afterInstall` flag tweaks the CTA copy slightly (omits "or Install hook").
- Sidebar CSS got a `.err.diag` block with title / body / detail (preformatted, max-height 90px scrollable) / actions / hint. Honors VS Code theme tokens via `--vscode-*`. Detail block uses `word-break: break-all` so long Windows paths don't blow the layout.
- The webview now uses `command:claudeUsage.openSettingsJson` from inline anchors; that command is a direct rename / consolidation of the previous inline "Open settings.json" branch inside `maybePromptSetup`. Simpler to maintain one open-settings entry point that all renderers can link to.
