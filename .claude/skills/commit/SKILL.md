...
Asset-only patch bumps (e.g. an `icon.png` swap that ticks the version from
`0.44.0` → `0.44.1`) are exempt: edit `package.json` only, no CHANGELOG /
DEV-NOTES entry needed.

### 4. Draft the commit message

Match the existing style. Inspect `git log --oneline -10` first. Typical
...
3. Run `git log -1 --format=fuller` and confirm the author line is the user
   (not Claude) and there is no AI-attribution trailer. For this repo the
   author must be `Timofei Nikitchenko <avhatar@gmail.com>` — never the
